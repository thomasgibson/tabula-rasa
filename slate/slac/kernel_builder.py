...
def prepare_tsfc_kernels(temps, tsfc_parameters=None):
    ...
    for expr in temps.keys():
        ...
        interior_facet_integrals = filter(lambda x: "interior_facet" in x.integral_type(), integrals)
        ...
        # Now for the rest:
        other_integrals = filter(lambda x: "interior_facet" not in x.integral_type(), integrals)
