...
A = Tensor(inner(2*nu*sym(grad(u)), grad(v))*dx
           + dot(-2*nu*sym(grad(u))*n + (2*nu*alpha/he)*u, v)*dSs
           - dot(2*nu*u, sym(grad(v))*n)*dSs)
B = Tensor(-dot(p, div(v))*dx)
C = Tensor(-alpha/he*2*nu*inner(ubar, v)*dSs
           + 2*nu*inner(ubar, sym(grad(v))*n)*dSs)
D = Tensor(dot(pbI*n, v)*dS)

# Right-hand side tensor
Q = Tensor(inner(f, v)*dx)

# Remaining momentum surface terms
K = Tensor(alpha/he * 2*nu*dot(ubar, vbar)*dSs)
L = Tensor(-dot(pbar*n, vbar)*dSs)

# Stabilization tensors
F = Tensor(-beta*he/(nu + 1) * dot(p, q)*dSs)
H = Tensor(beta*he/(nu + 1) * dot(pbar, q)*dSs)
P = Tensor(-beta*he/(nu + 1) * pbar * qbar * dSs)
